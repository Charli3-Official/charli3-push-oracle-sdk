...
class OracleOwner():
    ...
    def __init__(self,
        ...
        self.address = Address(payment_part=self.pub_key_hash,staking_part=self.stake_key_hash,
    ...
    def add_nodes(self, pkhs:List[str]):
        ...
        for node in pkhs:
            if not check_node_exists(aggstate_datum.aggstate.agSettings.osNodeList, node):
        ...
        if len(eligible_nodes) > 0 :
            ...
            updated_aggstate_datum = self._add_nodes_to_aggstate(aggstate_datum, eligible_nodes)

            builder = TransactionBuilder(self.context)
            builder.add_script_input(aggstate_utxo, redeemer=deepcopy(add_nodes_redeemer))
            ...
            node_nfts = MultiAsset.from_primitive(
                {
                    self.nft_hash.payload: {
                        b"NodeFeed": len(eligible_nodes),  # Name & Quantity of node nft to mint
                    }
                }
            )
            ...
            for node in eligible_nodes:
                node_datum = NodeDatum(node_state=NodeState(nodeOperator=NodeInfo(node),
                        nodeFeed=Nothing()))
                node_output = TransactionOutput(self.oracle_addr,
                                    Value(2000000, self.single_node_nft), datum=node_datum)
    ...
    def remove_nodes(self, pkhs:List[str]):
        ...
        # check if node already exist in aggstate node list
        for node in pkhs:
            if check_node_exists(aggstate_datum.aggstate.agSettings.osNodeList, node):
                eligible_nodes.append(node)

        if len(eligible_nodes) > 0 :
            # prepare datums, redeemers and new node utxos for eligible nodes
            remove_nodes_redeemer = Redeemer(RedeemerTag.SPEND, UpdateSettings())
            updated_aggstate_datum = self._remove_nodes_from_aggstate(aggstate_datum, eligible_nodes)

            builder = TransactionBuilder(self.context)
            builder.add_script_input(aggstate_utxo, redeemer=deepcopy(remove_nodes_redeemer))
            ...
            node_nfts = MultiAsset.from_primitive(
                {
                    self.nft_hash.payload: {
                        b"NodeFeed": -len(eligible_nodes),  # Negative sign indicates burning
                    }
                }
            )
            ...
            for node in eligible_nodes:
                ...
                builder.add_script_input(node_utxo, redeemer=deepcopy(remove_nodes_redeemer))
    ...
    def edit_settings(self, settings: OracleSettings):
        ...
        if (settings != aggstate_datum.aggstate.agSettings and
            settings.osNodeList == aggstate_datum.aggstate.agSettings.osNodeList) :
            ...
            builder = TransactionBuilder(self.context)
            builder.add_script_input(aggstate_utxo, redeemer=deepcopy(edit_settings_redeemer))
    ...
    def convert_datums_to_inlineable(self):
        ...
        aggstate_utxo: UTxO = filter_utxos_by_asset(oracle_utxos,self.aggstate_nft)[0]
        aggstate_datum: AggDatum = AggDatum.from_cbor(self.context._get_datum(aggstate_utxo))

        nodes_utxos: List[UTxO] = filter_utxos_by_asset(oracle_utxos, self.node_nft)
        node_utxos_with_datum: List[UTxO] = self.context.get_node_datums_with_utxo(nodes_utxos)
        ...
        for utxo in node_utxos_with_datum :
            builder.add_script_input(utxo,redeemer=deepcopy(aggregate_redeemer), datum= utxo.output.datum)
            tx_output = deepcopy(utxo.output)
            tx_output.datum_hash = None
            builder.add_output(tx_output)

        builder.add_script_input(aggstate_utxo, redeemer=deepcopy(aggregate_redeemer), datum=aggstate_datum)
    ...
    def _add_nodes_to_aggstate(self, aggstate_datum: AggDatum,nodes: List[bytes]) -> AggDatum:
        """add nodes to aggstate datum"""
        aggstate_datum.aggstate.agSettings.osNodeList.extend(nodes)
        return aggstate_datum

    def _remove_nodes_from_aggstate(self, aggstate_datum: AggDatum,nodes: List[bytes]) -> AggDatum:
        """remove nodes to aggstate datum"""
        for node in nodes:
            if node in aggstate_datum.aggstate.agSettings.osNodeList:
                aggstate_datum.aggstate.agSettings.osNodeList.remove(node)

        return aggstate_datum

    def _update_aggstate(self, aggstate_datum: AggDatum, settings: OracleSettings) -> AggDatum:
